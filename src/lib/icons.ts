import { languageExtensions } from "@/data/language-names";
...
export const getIconUrl = (languageName: LanguageName) => {
  const extensions = languageExtensions[languageName];
  const icon = fileIcons.icons.find((icon) => {
    if (icon.fileExtensions) {
      const index = icon.fileExtensions.findIndex((ext) =>
  ...
  if (icon) {
    return `/file-icons/${icon.name}.svg`;
  }
  return `/file-icons/${fileIcons.defaultIcon.name}.svg`;
};
